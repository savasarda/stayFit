...
function calculateDailyCalories(currentWeight: number, targetWeight: number, height = 0) {
  if (!currentWeight || !targetWeight) return 0
  const base = currentWeight * 24
  const activityEstimate = base * 1.35
  const direction = targetWeight < currentWeight ? -320 : targetWeight > currentWeight ? 220 : 0
  const heightAdjustment = height ? Math.max(-120, Math.min(120, (height - 170) * 4)) : 0
  return Math.round(Math.max(1200, Math.min(3400, activityEstimate + direction + heightAdjustment)) / 10) * 10
}

function getAutoCalorieText(currentWeight: string, targetWeight: string, height = '') {
  const calories = calculateDailyCalories(Number(currentWeight), Number(targetWeight), Number(height))
...
function OnboardingPage({ profile, onSave, onSignOut }: { profile: Profile; onSave: (profile: Profile) => void; onSignOut?: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    dailyCalories: String(profile.dailyCalories || calculateDailyCalories(profile.currentWeight || profile.startWeight, profile.targetWeight, profile.height) || ''),
  ...
  function update(field: keyof typeof form, value: string) {
    setForm((current) => {
      ...
      if (field === 'currentWeight' || field === 'targetWeight' || field === 'height') next.dailyCalories = getAutoCalorieText(next.currentWeight, next.targetWeight, next.height)
      return next
    })
  }
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const dailyCalories = calculateDailyCalories(currentWeight, targetWeight, height)
    const mealCount = Number(form.mealCount)
    if (!height || !currentWeight || !targetWeight || !dailyCalories || !mealCount) { setError('Devam etmek için boy, güncel kilo, hedef kilo ve öğün sayısını doldur.'); return }
...
function TodayPage({ profile, water, meals, dailyRecords, recordedDates, selectedDate, lastPhotoName, isMealDetailOpen, onSelectDate, onAddWater, onRemoveWater, onOpenMealDetail, onCloseMealDetail, onOpenAssistant, onOpenCamera, onOpenGallery, onAddManualMeal }: { profile: Profile; water: number; meals: Meal[]; dailyRecords: DailyRecords; recordedDates: Set<string>; selectedDate: string; lastPhotoName: string; isMealDetailOpen: boolean; onSelectDate: (date: string) => void; onAddWater: () => void; onRemoveWater: () => void; onOpenMealDetail: () => void; onCloseMealDetail: () => void; onOpenAssistant: () => void; onOpenCamera: () => void; onOpenGallery: () => void; onAddManualMeal: (meal: Meal) => void }) {
  ...
  return <>
    ...
    <section className="daily-hero" aria-label="Gün özeti">
      ...
      <div className="daily-mini-stats">
        ...
        <div><span className="mini-stat-ring blue" style={{ '--progress': `${waterProgress * 3.6}deg` } as CSSProperties}><Droplets size={14} /></span><strong>{formatWaterAmount(water)}</strong><small>Su</small></div>
    ...
    <section className="tracker-card water" aria-label="Su takibi"><span className="tracker-icon"><Droplets size={22} /></span><div className="water-summary"><p>Su takibi</p><strong>{formatWaterAmount(water)} / 2.5 L</strong><div className="water-progress"><span style={{ width: `${Math.min((water / 2500) * 100, 100)}%` }} /></div></div><div className="water-actions"><button type="button" onClick={onRemoveWater} disabled={water === 0} aria-label="250 mililitre su azalt">−</button><button type="button" onClick={onAddWater} aria-label="250 mililitre su ekle">+</button><small>250 ml</small></div></section>
